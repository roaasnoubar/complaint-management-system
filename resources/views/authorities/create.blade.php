...
@section('content')
<h1 class="text-2xl font-bold mb-6">Add Authority</h1>
<form method="POST" action="{{ route('authorities.store') }}" class="max-w-2xl space-y-4">
    @csrf
    <div><label class="block text-sm font-medium mb-1">Name *</label>
        <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-3 py-2 border rounded dark:bg-gray-800"></div>
    ...
    <div class="flex gap-2">
        ...
        <a href="{{ route('authorities.index') }}" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded">Cancel</a>
    </div>
</form>
@endsection
